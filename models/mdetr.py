...
class MDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, captions, encode_and_save=True, memory_cache=None):
        ...
        if encode_and_save:
            ...
            # features, pos = self.backbone(samples.tensors) #samples [1,3,672,711] features [6, 2048, 23, 33] 
            features, pos = self.backbone(samples) #samples [1,3,672,711] features [6, 2048, 23, 33] 
            src, mask = features[-1].decompose() #src:[1,2048,21,23] [1,21,23]
...
class SetCriterion(nn.Module):
    ...
    def loss_softkd(self, memory_cache, outputs, targets, positive_map, indices, num_boxes, **kwargs):
        ...
        if self.args.layer_wise_loss:
            img_memory_student = memory_cache_sth['img_memory']
            img_memory_teacher = memory_cache_noun['img_memory']

            # 检查形状是否一致
            min_length = min(img_memory_student.shape[0], img_memory_teacher.shape[0])
            img_memory_student = img_memory_student[:min_length]
            img_memory_teacher = img_memory_teacher[:min_length]
            
            # 归一化到概率分布
            img_memory_student = F.log_softmax(img_memory_student, dim=-1)
            img_memory_teacher = F.softmax(img_memory_teacher, dim=-1)
            
            # 计算KL散度损失
            loss_img_memory = F.kl_div(img_memory_student, img_memory_teacher, reduction='batchmean')
            loss_softkd += self.args.img_memory_distill_coef * loss_img_memory
...
def build(args):
    ...
    # backbone = build_internimage(args)
    ...
    if args.load_full == False and args.load_word_full == False:
