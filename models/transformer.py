...
from transformers import RobertaConfig, RobertaModel, RobertaTokenizerFast, AutoTokenizer, AutoModelForMaskedLM, AutoModelForTokenClassification, AutoModelForSequenceClassification, BertTokenizerFast
from IPython import embed
from .rpn import VisionLanguageFusionModel
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.args = args
        if self.args.fusion==True:
            self.vision_language_fusion = VisionLanguageFusionModel(256, 256, 8, use_dyhead = self.args.use_dyhead, use_txtlayer = self.args.use_txtlayer)
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            if isinstance(text[0], str):
                if self.args.verb_att == True:
                    # Encode the text
                    tokenized = self.tokenizer.batch_encode_plus(text, padding="longest", return_tensors="pt").to(device)

                    # Encode the first word (verb)
                    # tokenized_first = self.tokenizer_word.batch_encode_plus(text, padding="longest", return_tensors="pt").to(device)
                    # verb = [self.tokenizer_word.decode(tokenized_first['input_ids'][i][1]) for i in range(len(tokenized_first['input_ids']))]
                    
                    verb = [sentence.split(' ')[0] if sentence else '' for sentence in text]
                    tokenized_verb = self.tokenizer.batch_encode_plus(verb, padding="longest", return_tensors="pt").to(device)

                    encoded_text = self.text_encoder(**tokenized)
                    first_verb_feature = self.text_encoder(**tokenized_verb).last_hidden_state
                    ...
                    # Use first_word_feature as query, text_memory as key and value
                    text_memory_verb = self.attention(
                        query=text_memory.transpose(0, 1),
                        key=first_verb_feature,
                        value=first_verb_feature
                    )[0].transpose(0, 1)
                    text_memory = text_memory + text_memory_verb

                    # Resize the encoder hidden states to be of the same d_model as the decoder
                    # Change dimension order for batch normalization (expected input: (batch_size, num_features, ...))
                    text_memory_verb = text_memory_verb.permute(1, 2, 0)  # [batch_size, d_model, sequence_length]
                    text_memory_verb = self.bn(text_memory_verb)
                    # Revert dimension order after normalization
                    text_memory_verb = text_memory_verb.permute(2, 0, 1)
            ...
            if self.args.fusion==True:
                src, text_memory_resized = self.vision_language_fusion(src, text_memory_resized) 
            ...
            # image 和 text features进行拼接
            # src, text_memory_resized = self.vision_language_fusion(src, text_memory_resized) 
            # breakpoint()
            src = torch.cat([src, text_memory_resized], dim=0) #src: torch.Size([759, 6, 256]), text_memory_resized: torch.Size([7, 6, 256])
