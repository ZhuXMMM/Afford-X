...
def evaluate(
    ...
    for batch_dict in metric_logger.log_every(data_loader, 10, header):
        ...
        if "segm" in postprocessors.keys():
            ...
            print('results key', results[0].keys())
            print('results shape:', results[0]['masks'].shape)
...
def finetune(
    ...
    header = "Epoch: [{}]".format(epoch)
    print_freq = 10
    total_grad = {name: torch.zeros_like(param) for name, param in model.named_parameters()}
    num_training_steps = int(len(data_loader) * args.epochs)
    for i, batch_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        curr_step = epoch * len(data_loader) + i
        # example_rel = batch_dict["example_rel"]
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                total_grad[name] += param.grad.data.abs()
        ######################
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            grads.append(total_grad[name].sum())
        else:
            grads.append(torch.zeros_like(param).sum())
    sorted_grads = sorted(range(len(grads)), key=lambda k: grads[k], reverse=True)
    num_fixed_params = int(len(sorted_grads) * 0.6)


    return num_fixed_params, sorted_grads