...
# TasksAbbreviation=np.load("./data/todo.npy",allow_pickle=True).item()
# print(TasksAbbreviation)
def load_tasks(args):
    if args.load_word_full == True and args.load_full == True:
        TasksAbbreviation=np.load("./data/todo_lvis.npy",allow_pickle=True).item()
    elif args.load_word_full == True and args.load_full == False:
        TasksAbbreviation=np.load("./data/todo_coco.npy",allow_pickle=True).item()
    else:
        TasksAbbreviation={
                            1: 'step on ',
    ...
    return TasksAbbreviation

class CustomLVIS(LVIS):
    def __init__(self, annotation_path, root):
        super().__init__(annotation_path)
        self.root = root

    def _load_image(self, id: int) -> Image.Image:
        img_info = self.load_imgs([id])[0]
        coco_url = img_info['coco_url']
        path = coco_url.split('/')[-2] + '/' + coco_url.split('/')[-1]
        return Image.open(os.path.join(self.root, path)).convert("RGB")

class TdodDetection(torchvision.datasets.CocoDetection):
    def __init__(self, args, img_folder, root, ann_file, image_set, transforms, return_masks, return_tokens, tokenizer, is_train=False):
        self.args = args
        ...
        self.prepare = TOISTConvertCocoPolysToMask(self.catid2name, return_masks, return_tokens, tokenizer=tokenizer, args = args)
        self.return_masks = return_masks
        self.is_train = is_train
        if self.args.load_full:
            self.lvis = CustomLVIS(self.ann_file, self.root_lvis)
            self.ids = self.lvis.get_img_ids()
            self.catid2name = {str(cat['id']): cat['name'] for cat in self.lvis.load_cats(None)}

    def __getitem__(self, idx):
        if self.args.load_full:
            img_id = self.lvis.get_img_ids()[idx]
            img_ori = self.lvis._load_image(img_id)
        ...
        TasksAbbreviation = load_tasks(self.args)
...
class TOISTConvertCocoPolysToMask(object):
    def __init__(self, catid2name=None, return_masks=False, return_tokens=False, tokenizer=None, args=None):
...
def build(dataset_file, image_set, args, tokenizer):
    if image_set == 'train':
        if not args.load_full:
            ...
            img_dir = Path(args.coco_path) / "train2017"
            root = Path(args.coco_path)
        # img_dir = "/mnt/lustre/shared/dataset/coco2014/train2017"
        ...
        if not args.load_full:
            ...
            img_dir = Path(args.coco_path) / "val2017"
            root = Path(args.coco_path)
        # img_dir = "/mnt/lustre/shared/dataset/coco2014/val2014"
